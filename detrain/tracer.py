...
tokenizer = re.compile("[^a-zA-Z0-9]+")

white_list = [
    "tensorflow.python.util.lock_util._Context.__init__",
    "tensorflow.python.keras.callbacks.CallbackList._call_batch_hook",
    "tensorflow.python.util.object_identity.ObjectIdentitySet.__init__",
    "tensorflow.python.autograph.core.naming.Namer.__init__",
]
random_create = [
    "torch._C.Generator",
    "myrand.random_create.__init__",
...
class LineInfo:
    def __init__(
        ...
        self.caller_code = (
            caller_code.replace('"', "") if caller_code is not None else ""
        )
...
def is_random_function(name, func_obj, obj_info):
    if inspect.ismodule(func_obj) or inspect.isclass(func_obj):
        return False
...
class StatementInstance:
    def __init__(self, key, use, define, lineinfo):
        self.key = key
        self.use = use
        self.define = define
...
class CheckpointAnalyzer:
    ...
    def add_stmt(self, key, use, define, lineinfo):
        """
        key: str, filename:lineno
        """
        self.stmt.append(StatementInstance(key, use, define, lineinfo))
...
def hit_checkpoint(vars):
    global loop_time
    loop_time += 1
    print("================ANALYSIS==============")
    print("checked variable(s): " + str(vars))
    for stmt in reversed(checkpoint_analyzer.stmt):
        print(
            "%s: %s: use:%s define:%s"
            % (stmt.key, stmt.lineinfo, stmt.use, stmt.define)
        )
    print("================END-ANALYSIS==============")
...
def process_event(frame, event, arg):
    ...
    if (
        (finfo is not None)
        and (finfo.code_context is not None)
        and (finfo.index is not None)
    ):
    ...
    if event in ["call", "c_call"]:
        ...
        for k, v in local_vars.items():
            if k not in [
                ...
                try:
                    ...
                    if is_random_function(k, v, None):
                        rngs[k] = v
        ...
        for rng in random_create:
            if rng in full_name:
                ...
                process_event.generators[obj_id] = (target, None)
                break
            ...
            if func in full_name:
                ...
                for i in range(len(callstack) - 1, -1, -1):
                    if callstack[i][0] == "LOOP":
                        # add training loop
                        loop_key = (
                            callstack[i][3].file_name
                            + ":"
                            + str(callstack[i][3].line_no)
                        )
                        if loop_key not in process_event.loops:
                            ...
                            process_event.loops[loop_key] = (
                                count,
                                process_event.loops[loop_key][1],
                            )

                        process_event.loop_trace.append(
                            ("WEIGHTS", current_info, str(callstack))
                        )
                        break
                stop_trace()
                restart_trace()
    ...
    # end of call event
        ...
        if len(callstack) >= 1:
            cur_cs = callstack[-1]
        code_str = exe_code.strip()
        code_seg = code_str.split()
        # if cnt < 5:
        print(code_seg)
        if len(code_seg) > 0:
            ...
            if cur_cs[0] == "LOOP" and indent <= cur_cs[2]:
                ...
                while (
                    (len(callstack) > 0)
                    and (callstack[-1][0] == "LOOP")
                    and (indent <= callstack[-1][2])
                ):
                    loop_info = callstack.pop()
                    loop_key = loop_info[3].file_name + ":" + str(loop_info[3].line_no)
                    if loop_key in process_event.loops:
            ...
            if code_seg[0] in ["for", "while"]:
                ...
                if loop_key in process_event.loops:
                    ...
                    process_event.loop_trace.append(
                        ("LOOP", current_info, str(callstack))
                    )

            if "=" in code_str:
                lhs_rhs = code_str.split("=")
                lhs = lhs_rhs[0]
                rhs = lhs_rhs[1]
                lhs = set(tokenizer.sub(" ", lhs).split())
                rhs = set(tokenizer.sub(" ", rhs).split())
                print("LHS: " + str(lhs))
                print("RHS: " + str(rhs))
                use = set()
                define = set()
                for k, _ in local_vars.items():
                    if k in rhs:
                        use.add(k)
                    if k in lhs:
                        define.add(k)
                ...
                if loop_time >= 1:
                    key = "%s:%d" % (current_info.file_name, current_info.line_no)
                    checkpoint_analyzer.add_stmt(
                        key, use=use, define=define, lineinfo=current_info
                    )
        return mytrace
    # end of line event
    if module_name != "":
        # TODO XXZ
        cnt += 1
        if (cnt % 1000 == 0) or cfg.detailed_trace:
            stop_trace()
            print(cnt)
...
def save_trace():
    ...
    print("LOOPS: " + str(process_event.loops))
